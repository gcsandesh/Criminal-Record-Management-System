...
export default function RecordPreview({
	...
	// get crime
	useEffect(() => {
		crimeId && fetch(`http://localhost:9988/api/crimes/get/id/${crimeId}`)
			.then((res) => res.json())
		...
		>
			<Link to={`${recordId}`}>
				<div className="mx-auto flex gap-1 w-full">
					...
					<span className="w-1/12">Sex: {gender ? gender.name : "-"}</span>
					<span className="w-2/12">Height: {height} in</span>
					<span className="w-2/12">Crime: {crime ? crime.name : "-"}</span>
				</div>
			</Link>
		</div>
	)
